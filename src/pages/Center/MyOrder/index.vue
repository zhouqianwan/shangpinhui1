  ...
  <div>
    <div class="order-content">
          ...
          <tbody>
            <tr v-for="(goods,index) in order.orderDetailList" :key="goods.id">
              ...
              <td :rowspan="order.orderDetailList.length" v-if="index == 0" width="13%" class="center">
                <ul class="unstyled">
                  <li>总金额¥{{goods.splitTotalAmount}}.00</li>
                  <li>在线支付</li>
      ...
      <div class="choose-order">
        <Pagination :pageNo='this.page' :pageSize="this.limit" :total="this.orderInfo.total" :continues="3" @getPageNo="getPageNo"></Pagination>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getData() {
      let { page, limit } = this
      let res = await this.$api.reqGetMyOrder(page, limit)
      if (res.code == 200) {
